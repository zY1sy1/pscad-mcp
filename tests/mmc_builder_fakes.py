"""Recording public-boundary fake used by the MMC lifecycle tests."""
...
class RecordingMmcService:
    def __init__(self, *, fail_on: str | None = None, output: Any = None, run_statuses: list[str] | None = None) -> None:
        ...
        self.reset_statuses_each_run = run_statuses is None
        self.run_statuses = list(["running", "completed"] if run_statuses is None else run_statuses)
    ...
    def _write_project(self, path: Path, project_name: str | None = None) -> None:
        ...
        ET.SubElement(root, "definition", {"name": "Main"})
    ...
    async def add_canvas_component(
        ...
        *,
        ...
        return {"id": component_id, "name": logical_id, "definition": f"{library}:{name}", "location": {"x": x, "y": y}, "orientation": orientation}
    ...
    async def create_connection(self, project_name: str, p1: list[int], p2: list[int], label: str | None, electrical: bool | None, *, canvas_name: str = "Main") -> dict[str, Any]:
        self._call("create_connection", project_name, p1, p2, label, electrical, canvas_name=canvas_name)
        return {"p1": p1, "p2": p2, "label": label}

    async def create_output_channel(self, project_name: str, path: str, units: str, *, call_id: int | None = None) -> dict[str, Any]:
    ...
    async def run_project(self, project_name: str) -> str:
        ...
        if not self.run_statuses and self.reset_statuses_each_run:
            self.run_statuses = ["running", "completed"]
        return "started"
