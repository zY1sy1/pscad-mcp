...
完整工具库存为 105 = 60 个通用工具 + 2 个拓扑工具 + 10 个 HVDC 工具 + 3 个学习工具 + 8 个 LCC 工具（含 blank 生命周期） + 6 个参数化 LCC 工具 + 11 个 MMC 工具（含 blank 生命周期） + 4 个通用 Blueprint Builder 工具 + 1 个始终注册的 `get_pscad_capabilities` 能力工具；原有 60 个通用工具的名称和默认返回形状保持不变。
其中 104 个兼容/领域工具 + 1 个始终注册的能力工具。
...
验收失败后的处理遵循[验收准则](../acceptance-criteria.md)：一次失败只是本次尝试的结果，必须继续区分环境争用、实现缺陷、模型物理问题与外部阻塞，修复后重新验收，不能直接结束任务或降低指标以取得通过。

更新后的 legacy、拓扑、LCC native/fixed 和程序预检支持独立实例并发验收。在每个任务自己的进程环境中设置 `PSCAD_MCP_ACCEPTANCE_CONCURRENT=1`，并使用独立工程和输出目录；清理只检查本任务的受管 PID。默认仍是独占模式。该设置不授予额外许可证，不适用于尚未更新的旧 worktree，也不允许多个任务共同操作一个 PSCAD 实例。
...
`read_documentation` 支持 `offset`、`max_chars`。同步后的模块也可通过
`pscad-docs://modules/` MCP 资源读取。

生成的 API 文档仅在调用 `sync_documentation` 后写入本地状态。Windows 默认
目录为 `%LOCALAPPDATA%\\pscad-mcp\\docs`，`PSCAD_MCP_DOCUMENTATION_DIR` 可
覆盖，但必须是绝对路径。
...
工具数量应输出 `105 105`。真实 PSCAD 5.x 必须在安装并运行对应版本后另做相同级别验收。
