"""Deterministic grid routing primitives for the LCC blueprint."""
...
def _layout_error(message: str, **details: Any) -> BackendError:
    return BackendError("LCC_LAYOUT_INVALID", message, "hvdc", "validate_lcc_layout", details)
...
def absolute_port(
    origin: tuple[int, int], offset: tuple[int, int], orientation: int
) -> tuple[int, int]:
    if len(origin) != 2 or len(offset) != 2:
        raise _layout_error("origin and offset require two coordinates.")
    origin_point = (
        _coordinate(origin[0], "origin.x"),
        _coordinate(origin[1], "origin.y"),
    )
...
def validate_orthogonal_route(vertices: Sequence[Sequence[int]]) -> tuple[tuple[int, int], ...]:
    ...
    for index, vertex in enumerate(vertices):
        ...
        parsed.append(
            (
                _coordinate(vertex[0], f"vertices[{index}].x"),
                _coordinate(vertex[1], f"vertices[{index}].y"),
            )
        )
...
def route_intersects_rectangles(
    vertices: Sequence[Sequence[int]], rectangles: Sequence[Any]
) -> None:
    ...
    for rectangle_index, rectangle_value in enumerate(rectangles):
        ...
        for point in route:
            if left < point[0] < right and top < point[1] < bottom:
                raise _layout_error(
                    "route enters a declared component rectangle.",
